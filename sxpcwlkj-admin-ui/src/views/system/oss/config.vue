  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="配置key" prop="configKey">
        <el-select v-model="queryParams.configKey" placeholder="请选择配置key" clearable>
          <el-option
    ...
    <el-table v-loading="loading" :data="ossConfigList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="桶名称" align="center" prop="bucketName" v-if="columns[3].visible" />
      <el-table-column label="前缀" align="center" prop="prefix" v-if="columns[4].visible" />
      <el-table-column label="域" align="center" prop="region" v-if="columns[5].visible" />
...
const columns = ref([
  ...
  { key: 3, label: `桶名称`, visible: true },
  { key: 4, label: `前缀`, visible: true },
  { key: 5, label: `域`, visible: true },
...
/** 用户状态修改  */
function handlePreviewListResource(row) {
...
function handleStatusChange(e){
  form.value=e;
  if (form.value.ossConfigId != null) {
        changeOssConfigStatus(e.ossConfigId,e.status,e.configKey).then(response => {
          //proxy.$modal.msgSuccess("修改成功");
          //open.value = false;
          getList();
        }).finally(() => {
